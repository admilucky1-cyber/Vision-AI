"""
Vision AI v8.8.1 - Production-Grade Multi-Modal AI Assistant
============================================================
...
try:
    ...
    shared_limiter = None  # type: ignore
    _HAS_SLOWAPI = False
...
APP_VERSION = (BASE_DIR / "VERSION").read_text(encoding="utf-8").strip() if (BASE_DIR / "VERSION").exists() else "8.8.1"
...
async def _rl_handler(request: Request, exc: Exception):
    """Always-defined 429 handler (no dependency on slowapi private APIs)."""
    detail = getattr(exc, "detail", None) or str(exc) or "Too many requests"
    return JSONResponse(
        status_code=429,
...
app.add_exception_handler(429, _rl_handler)
...
async def add_security_headers(request: Request, call_next):
    """Security headers + smart robots/cache (public pages must stay indexable)."""
    ...
    # Private/API paths: noindex. Public HTML: allow indexing for SEO.
    private = (
        path.startswith("/api/")
...
if not _cors_origins:
    _cors_origins = ["http://localhost:5050", "http://127.0.1.1:5050"]
...
# Compression: Brotli (if installed) + Gzip — Zstd > Brotli > Gzip > none
try:
    from services.compression_mw import mount_compression
    _cmp = mount_compression(app)
    logging.getLogger("vision-ai").info("Compression enabled: %s", ", ".join(_cmp) or "none")
except Exception as _cmp_err:
    app.add_middleware(GZipMiddleware, minimum_size=500, compresslevel=6)
    logging.getLogger("vision-ai").warning("Brotli mount skipped, Gzip only: %s", _cmp_err)

# TrustedHost: allow all on Railway unless ALLOWED_HOSTS is an explicit list
_raw_hosts = AppConfig.ALLOWED_HOSTS or ["*"]
_railway = (os.getenv("RAILWAY_PUBLIC_DOMAIN") or os.getenv("RAILWAY_STATIC_URL") or "").strip()
_trusted = list(_raw_hosts)
if _railway:
    host_only = _railway.replace("https://", "").replace("http://", "").split("/")[0]
    if host_only and host_only not in _trusted:
        _trusted.append(host_only)
# Always permit Railway / health probes when wildcard requested
...
try:
    ...
    logging.getLogger("vision-ai").warning("TrustedHostMiddleware skipped: %s", _th_err)
...
# Settings API (server-authoritative preferences)
try:
    from routes.settings_api import router as settings_api_router
    app.include_router(settings_api_router)
    logger.info("✅ Settings API mounted at /api/settings")
except Exception as _se:
    logger.error("Settings API mount failed: %s", _se)
...
try:
    ...
    logger.warning("DB init skipped: %s", _db_e)
...
try:
    from routes.data_lab import router as data_lab_router
    app.include_router(data_lab_router)
    logger.info("✅ Data Lab mounted at /api/data-lab")
except Exception as _dl_err:
    logging.getLogger("vision-ai").warning("Data Lab router skipped: %s", _dl_err)

try:
    from routes.llm_api import router as llm_api_router
    app.include_router(llm_api_router)
    logger.info("✅ LLM API mounted at /api/llm")
except Exception as _llm_err:
    logging.getLogger("vision-ai").warning("LLM API router skipped: %s", _llm_err)

try:
    from routes.smart_video import router as smart_video_router
    app.include_router(smart_video_router)
    logger.info("✅ Smart Video mounted at /api/smart-video")
except Exception as _sv_err:
    logging.getLogger("vision-ai").warning("Smart Video router skipped: %s", _sv_err)

try:
    from routes import models as models_routes
    app.include_router(models_routes.router)
except Exception as _me:
    logger.error("Models router mount failed: %s", _me)

# RAG Engine + routing profiles
try:
    from routes.rag_engine import router as rag_engine_router
    app.include_router(rag_engine_router)
    logger.info("✅ RAG Engine mounted at /api/rag")
    try:
        from services.rag_engine_core import health_check as _rag_health
        _rh = _rag_health()
        logger.info("RAG health: ok=%s chroma=%s", _rh.get("ok"), (_rh.get("chroma") or {}).get("available"))
    except Exception as _rhe:
        logger.warning("RAG health check: %s", _rhe)

except Exception as _re:
    logger.error("RAG Engine mount failed: %s", _re)

try:
    from services.routing_profiles import list_profiles
    @app.get("/api/routing/profiles")
    async def api_routing_profiles():
        return {"ok": True, "profiles": list_profiles()}
    logger.info("✅ Routing profiles at /api/routing/profiles")
    @app.get("/api/routing/analytics")
    async def api_routing_analytics():
        from services.routing_analytics import snapshot
        return snapshot()
...
async def health_ready():
    ...
    try:
        ...
        db_ok = False
    return {"status": "ready" if db_ok else "degraded", "database": db_ok, "version": APP_VERSION}


@app.middleware("http")
async def utf8_charset_middleware(request, call_next):
    """Ensure JSON/text responses declare UTF-8 (avoids mojibake like Iâm)."""
    response = await call_next(request)
    ct = response.headers.get("content-type") or ""
    if ct.startswith("application/json") and "charset" not in ct.lower():
        response.headers["content-type"] = "application/json; charset=utf-8"
    elif ct.startswith("text/plain") and "charset" not in ct.lower():
...
async def service_worker_root():
    ...
    path = Path(__file__).resolve().parent / "frontend" / "sw.js"
    if path.is_file():
...
async def web_manifest():
    from fastapi.responses import FileResponse
    path = Path(__file__).resolve().parent / "frontend" / "manifest.webmanifest"
    if path.is_file():
...
try:
    from routes.seo_public import router as seo_public_router
    app.include_router(seo_public_router)
except Exception as _seo_err:
    logger.warning("SEO router skipped: %s", _seo_err)

try:
    from routes.device_api import router as device_api_router
    app.include_router(device_api_router)
except Exception as _dev_err:
    logger.warning("device_api router skipped: %s", _dev_err)
...
@app.get("/api/mongo/status", tags=["System"])
async def mongo_status():
    try:
        from services.mongo_store import status as mongo_status_fn
        return mongo_status_fn()
    except Exception as e:
        return {"configured": False, "available": False, "error": str(e), "backend": "sql_fallback"}
...
async def health_check_detailed():
    ...
    health_data = {
        ...
        "middlewares": [
            "CORS", "Compression(Brotli+Gzip)", "TrustedHosts", "RateLimiting", "SecurityHeaders",
...
@app.get("/data-lab")
@app.get("/data-lab.html")
async def page_data_lab():
    path = AppConfig.FRONTEND_DIR / "data-lab.html"
    if path.exists():
        return FileResponse(str(path), media_type="text/html")
    raise HTTPException(status_code=404, detail="data-lab.html missing")
...
try:
    from routes.engine_v53 import router as engine_v53_router
    app.include_router(engine_v53_router)
    logging.getLogger("vision-ai").info("✅ Engine v5.3 mounted at /api/engine")
except Exception as _e53:
    logging.getLogger("vision-ai").error("Engine v5.3 mount failed: %s", _e53)


try:
    from routes.forge_v54 import router as forge_v54_router
    app.include_router(forge_v54_router)
    logging.getLogger("vision-ai").info("✅ Forge v5.4 mounted at /api/forge")
except Exception as _f54:
    logging.getLogger("vision-ai").error("Forge v5.4 mount failed: %s", _f54)

@app.exception_handler(Exception)
async def vision_self_heal(request: Request, exc: Exception):
    """Self-healing: never crash session UI on unhandled errors."""
    import logging as _logging
    _logging.getLogger("vision-ai.selfheal").exception("Unhandled: %s %s", request.url.path, exc)
    ...
    return JSONResponse(
        ...
        content={
            ...
            "message": "Something went wrong. Your session is safe — retry or simplify the request.",
