...
def _print_banner() -> None:
    ver = "8.2.3"
    try:
        vp = Path(__file__).resolve().parent / "VERSION"
        if vp.exists():
            ver = vp.read_text(encoding="utf-8").strip() or ver
    except Exception:
        pass
    print(f"Vision AI v{ver} — starting (run.py)")
...
def main() -> None:
    ...
    # Railway / low-RAM: never multi-worker (doubles import memory)
    if os.environ.get("RAILWAY_ENVIRONMENT") or os.environ.get("RAILWAY_PROJECT_ID"):
        workers = 1
    host = os.environ.get("HOST", "0.0.0.0").strip() or "0.0.0.0"
    print(f"Starting Vision AI via run.py on {host}:{port} workers={workers}", flush=True)
    # Import app after print so logs show we reached bind path
    try:
        import main as _main_mod  # noqa: F401 — surface import errors in logs
        print("main:app import OK", flush=True)
    except Exception as _imp_e:
        print(f"FATAL: main:app import failed: {_imp_e}", flush=True)
        raise
