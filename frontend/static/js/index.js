...
function clearTokens(opts) {
    ...
    try {
        ...
        if (opts.forceLogin) {
            try {
                ...
                localStorage.setItem('vision_prefer_guest', '0');
            } catch (e) {}
        } else {
            try {
    ...
    try { if (window.VisionAuth && window.VisionAuth.clearTokens && opts.forceLogin) window.VisionAuth.clearTokens({ forceLogin: true }); } catch (e) {}
}
...
window.closeMobileSidebar = function () {
    try { if (window.VAStore) window.VAStore.setState({ sidebarOpen: false }); } catch (e) {}
    if (window.__vaSidebar && window.__vaSidebar.close) return window.__vaSidebar.close();
}
...
let chatHistory = (_safeLoadHistory() || []).map(c => {
...
function _closeAllChatMenus() {
    document.querySelectorAll(".chat-action-menu.show").forEach(el => el.classList.remove("show"));
    document.querySelectorAll(".chat-item-wrapper.menu-open").forEach(el => el.classList.remove("menu-open"));
}
...
function renderHistory() {
    ...
    const groups = {};
    const now = new Date();
    const cur = _chatId(currentChatId);
    (chatHistory || []).forEach(item => {
        if (!item || item.id == null) return;
        item.id = _chatId(item.id);
    ...
    order.forEach(group => {
        ...
        groups[group].forEach(item => {
            const id = _chatId(item.id);
            const wrapper = document.createElement("div");
            wrapper.className = "chat-item-wrapper" + (id === cur ? " active" : "");
            wrapper.dataset.chatId = id;

            const titleSpan = document.createElement("span");
            titleSpan.className = "chat-title";
            titleSpan.textContent = item.title || "New Chat";
            titleSpan.title = item.title || "New Chat";
...
function toggleChatMenu(btn, chatId) {
    const id = _chatId(chatId || (btn && btn.dataset.chatId));
    const wrapper = btn ? btn.closest(".chat-item-wrapper") : null;
    const menu = wrapper ? wrapper.querySelector('.chat-action-menu[data-chat-id="' + id.replace(/"/g, "") + '"]') : (btn && btn.nextElementSibling);
    document.querySelectorAll(".chat-action-menu.show").forEach(el => {
        if (el !== menu) el.classList.remove("show");
    });
    document.querySelectorAll(".chat-item-wrapper.menu-open").forEach(el => {
        if (el !== wrapper) el.classList.remove("menu-open");
    });
    if (menu) {
        menu.classList.toggle("show");
        if (wrapper) wrapper.classList.toggle("menu-open", menu.classList.contains("show"));
    }
}

function loadChat(id) {
    id = _chatId(id);
    const chat = chatHistory.find(c => _chatId(c.id) === id);
    if (!chat) return;
    currentChatId = id;
    ...
    (chat.messages || []).forEach(m => {
        try {
            if (typeof appendMessage === "function") appendMessage(m.role || "assistant", m.text || "", m);
            else if (typeof addMessageToUI === "function") addMessageToUI(m);
        } catch (err) {
            console.warn("loadChat message render", err);
        }
    });
    renderHistory();
    if (typeof isMobileViewport === "function" && isMobileViewport() && typeof closeMobileSidebar === "function") {
        closeMobileSidebar();
    }
}
...
function renameChat(id) {
    ...
    const chat = chatHistory.find(c => _chatId(c.id) === id);
    if (!chat) {
        console.warn("renameChat: id not found", id);
        return;
    }
    const newName = prompt("Rename chat:", chat.title || "New Chat");
    if (newName && newName.trim()) {
        chat.title = newName.trim().slice(0, 80);
        saveHistory();
        renderHistory();
        if (id === _chatId(currentChatId)) {
            const titleEl = document.getElementById("chatTitle");
...
function deleteChat(id) {
    ...
    const chat = chatHistory.find(c => _chatId(c.id) === id);
    if (!chat) {
        console.warn("deleteChat: id not found", id, chatHistory.map(c => c.id));
        return;
    }
    const label = (chat.title || "this chat").slice(0, 40);
    if (!confirm('Delete "' + label + '"?')) return;

    const wasCurrent = id === _chatId(currentChatId);
    chatHistory = chatHistory.filter(c => _chatId(c.id) !== id);
    saveHistory();
    _closeAllChatMenus();

    if (wasCurrent) {
        // Prefer next existing chat — do NOT attach delete to a brand-new empty chat incorrectly
        if (chatHistory.length) {
    ...
    if (role === 'ai' && typeof text === 'string') {
        ...
        if (typeof marked !== 'undefined' && typeof marked.parse === 'function') {
            try {
                ...
                const parsedHtml = marked.parse(fixMojibake(src));
...
function stopGeneration() {
    ...
    if (sendBtn) sendBtn.disabled = false;
        try { if (window.VAStore) window.VAStore.setState({ sending: false }); } catch (_va) {}
...
async function sendMessage() {
    ...
    try {
            ...
            try {
                ...
                if (sRes.ok && sRes.body) {
                    ...
                    function flushStreamRender() {
                        ...
                        if (!bubble) return;
                        try {
                          var html = (typeof marked !== 'undefined') ? marked.parse(fixMojibake(acc)) : acc;
        ...
        removeLoading();
        if (res.status === 405) {
            if (typeof addMessage === 'function') addMessage('ai', '❌ Chat endpoint returned 405 Method Not Allowed. Please redeploy the latest Vision AI build (POST /chat/send).');
            if (sendBtn) sendBtn.disabled = false;
        try { if (window.VAStore) window.VAStore.setState({ sending: false }); } catch (_va) {}
            if (stopBtn) stopBtn.hidden = true;
            return;
        ...
        if (stopBtn) stopBtn.hidden = true;
        if (res.ok) {
            var _ans = (data && data.answer != null) ? String(data.answer) : '';
            if (!_ans.trim()) {
                _ans = data.detail || data.error || 'No text returned from the model. Try another model in the menu, or check API keys on the server.';
                data.answer = _ans;
            }
            addMessage(
                'ai', 
...
function renderUserProfile() {
    ...
    const plan = localStorage.getItem('vision_ai_plan') || userData.plan || 'free';
    const hasToken = !!(window.VisionAuth && typeof VisionAuth.getAccessToken === 'function' && VisionAuth.getAccessToken())
        || !!(localStorage.getItem('vision_ai_access_token') || localStorage.getItem('vision_ai_token'));
    const isGuest = !!(userData.is_guest || userData.isGuest
        || localStorage.getItem('vision_ai_is_guest') === '1'
        || !userData.username
        || String(userData.username).toLowerCase() === 'guest');

    // Always rebuild a consistent shell so menu + logout stay present
    const nameToUse = (isGuest || !hasToken)
        ? 'Guest'
        : (userData.full_name || userData.username || 'User');
    const initials = (isGuest || !hasToken)
        ? 'G'
        : (String(nameToUse).split(/\s+/).map(function (n) { return n[0] || ''; }).join('').toUpperCase().slice(0, 2) || 'U');
    const planLabel = (isGuest || !hasToken)
        ? 'Free · sign in'
        : (String(plan).charAt(0).toUpperCase() + String(plan).slice(1));

    footer.innerHTML = `
        <div class="profile-menu-shell">
          <button type="button" class="profile-trigger" id="profileButton" aria-haspopup="true" aria-expanded="false"
              onclick="if(window.toggleProfileDropdown)toggleProfileDropdown(event)" title="Account">
            <span class="profile-avatar" aria-hidden="true">${initials}</span>
            <span class="profile-meta">
              <span class="profile-name">${escapeHtml(nameToUse)}</span>
              <span class="profile-plan">${escapeHtml(planLabel)}</span>
            </span>
            <svg class="profile-chevron" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path d="M6 9l6 6 6-6"/></svg>
          </button>
          <div class="profile-dropdown" id="profileDropdown" role="menu" aria-hidden="true" style="display:none;">
            <div class="profile-dd-head">${escapeHtml(nameToUse)}</div>
            <a role="menuitem" href="/settings.html">Account settings</a><a role="menuitem" href="/boost.html">Boost</a><a role="menuitem" href="/settings.html#appearance">Themes</a>
            <a role="menuitem" href="/usage.html">Usage</a>
            <a role="menuitem" href="/upgrade.html">Plans</a>
            <a role="menuitem" href="/versions.html">Versions</a>
            <button type="button" class="dl-item" role="menuitem" onclick="if(window.downloadCurrentChat)downloadCurrentChat()">Download chat</button>
            <hr class="menu-sep">
            ${(isGuest || !hasToken)
              ? '<a role="menuitem" href="/login.html">Sign in</a>'
    ...
    if (window.ProfileController && typeof ProfileController.bind === 'function') {
        try { ProfileController.bind(); } catch (e) {}
    }
}
...
async function ensureGuestSession() {
    try {
        ...
        try { localStorage.setItem('vision_prefer_guest', '1'); } catch (ePref) {}
        return true;
    } catch (e) {
...
async function checkAuth() {
    ...
    try {
        ...
        if (!res.ok) {
            // Invalid/expired token → soft clear (do NOT force login / skip guest)
            try {
              if (window.VisionAuth && window.VisionAuth.clearTokens)
                window.VisionAuth.clearTokens({});
              else {
                localStorage.removeItem('vision_ai_access_token');
                localStorage.removeItem('vision_ai_refresh_token');
              }
            } catch (eClr) { try { clearTokens(); } catch (e2) {} }
            try {
              sessionStorage.removeItem('vision_skip_guest');
              sessionStorage.removeItem('vision_force_login');
            } catch (eSk) {}
...
function copyPrompt(text) {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
        (window.vaCopyText ? window.vaCopyText(text) : navigator.clipboard.writeText(text)).then(doCopy).catch(() => {
            // fallback
...
/* v8.3.3 — quieter global handlers (no spam toasts on minor script noise) */
window.onerror = function(msg, url, line, col, error) {
    console.error('Global error:', msg, url, line, error);
    try {
        var s = String(msg || '');
        // Ignore common non-fatal noise (extensions, ads, aborted fetches)
        if (/Script error|ResizeObserver|Loading CSS|adsbygoogle|extension:\/\/|chrome-extension/i.test(s)) return true;
        if (window.__vaLastErrorToast && Date.now() - window.__vaLastErrorToast < 8000) return true;
        window.__vaLastErrorToast = Date.now();
        if (typeof showToast === 'function') {
            showToast('Something went wrong. You can keep chatting — or refresh if it continues.', 'error', 4500);
        }
    } catch (e) {}
    return true;
};
...
window.addEventListener('unhandledrejection', function(event) {
    ...
    try {
        var s = String((reason && reason.message) || reason || '');
        if (/AbortError|The user aborted|Failed to fetch|NetworkError|adsbygoogle|Load failed/i.test(s)) {
            // Network blips: do not scare the user with a global toast
            return;
        }
        if (window.__vaLastErrorToast && Date.now() - window.__vaLastErrorToast < 8000) return;
        window.__vaLastErrorToast = Date.now();
        if (typeof showToast === 'function') {
            showToast('Request issue — check connection or try again.', 'error', 4000);
        }
    } catch (e) {}
});
...
function copyMessageText(btn) {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
        (window.vaCopyText ? window.vaCopyText(text) : navigator.clipboard.writeText(text)).then(done).catch(() => {
            const ta = document.createElement('textarea');
...
function _getSpeechRecognition() {
    return (window.vaGetSpeechRecognition ? window.vaGetSpeechRecognition() : null) || window.SpeechRecognition || window.webkitSpeechRecognition || null;
}
...
(function () {
  if (window.__vaLogoutClickBound) return;
  window.__vaLogoutClickBound = true;
  document.addEventListener('click', function (ev) {
    var t = ev.target;
    if (!t || !t.closest) return;
    var el = t.closest("[data-action='logout'], .logout-item, #profileLogoutBtn");
    if (!el) return;
    ev.preventDefault();
    ev.stopPropagation();
    if (window.closeProfileDropdown) window.closeProfileDropdown();
    if (window.performLogout) window.performLogout();
    else if (window.VisionAuth && VisionAuth.logout) VisionAuth.logout({ confirm: true });
    else { try { localStorage.clear(); } catch (e) {} location.href = '/login.html'; }
  }, true);
})();


/* Sketch workspace modes: Chat / Image Gen / Video Gen */
(function () {
  function setMode(mode) {
    mode = mode || 'chat';
    document.body.setAttribute('data-ws-mode', mode);
    try { localStorage.setItem('vision_ws_mode', mode); } catch (e) {}
    ...
    var ta = document.getElementById('message');
    if (ta) {
      if (mode === 'image') ta.placeholder = 'Describe an image to generate…';
      else if (mode === 'video') ta.placeholder = 'Describe a video to generate…';
      else ta.placeholder = 'Ask anything, drop an image, or tap the mic…';
    }
  }
  function bind() {
    var root = document.getElementById('workspaceModes');
    if (!root || root._bound) return;
    root._bound = true;
    root.addEventListener('click', function (ev) {
      var btn = ev.target && ev.target.closest && ev.target.closest('.mode-tab');
      if (!btn) return;
      setMode(btn.getAttribute('data-mode') || 'chat');
    });
    var saved = 'chat';
    try { saved = localStorage.getItem('vision_ws_mode') || 'chat'; } catch (e) {}
    setMode(saved);
  }
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', bind);
  else bind();
  window.setWorkspaceMode = setMode;
})();

/* v6.7.2 — independent Online/Offline from /health */
(function visionHealthPing() {
  function ping() {
    fetch("/health", { credentials: "same-origin", cache: "no-store" })
      .then(function (r) { if (typeof updateStatus === "function") updateStatus(!!r.ok); })
      .catch(function () { if (typeof updateStatus === "function") updateStatus(false); });
  }
  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", function () { ping(); setInterval(ping, 30000); });
  else { ping(); setInterval(ping, 30000); }
})();

/* v6.8.1 — ensure theme controls receive clicks */
(function visionThemeUnblock() {
  function bind() {
    var ids = ["themePickerBtn", "headerThemeBtn", "floatingThemeToggle", "themeToggle"];
    ids.forEach(function (id) {
      var el = document.getElementById(id);
      if (!el) return;
      el.style.pointerEvents = "auto";
      el.style.zIndex = "10051";
      if (!el.getAttribute("data-theme-bound")) {
        el.setAttribute("data-theme-bound", "1");
        el.addEventListener("click", function (e) {
          e.stopPropagation();
          if (id === "themePickerBtn") return; // panel handler owns this
...
/* v823-menu-global-close */
(function () {
  if (window.__vaMenuCloseBound) return;
  window.__vaMenuCloseBound = true;
  document.addEventListener("click", function (e) {
    if (e.target.closest && (e.target.closest(".chat-action-menu") || e.target.closest(".chat-item-options"))) return;
    if (typeof _closeAllChatMenus === "function") _closeAllChatMenus();
  }, true);
  document.addEventListener("keydown", function (e) {
    if (e.key === "Escape" && typeof _closeAllChatMenus === "function") _closeAllChatMenus();
  });
})();
