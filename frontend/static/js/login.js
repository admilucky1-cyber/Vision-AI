...
(function () {
  ...
  function init() {
    // After browser close: if user used guest before, return to chat without forcing login form
    try {
      var force = sessionStorage.getItem("vision_force_login") === "1";
      var preferGuest = localStorage.getItem("vision_prefer_guest") !== "0";
      var tok0 = localStorage.getItem("vision_ai_access_token");
      if (!force && preferGuest && !tok0) {
        // Silent guest then home — avoids "always ask login" after close
        handleGuest();
        return;
      }
    } catch (eBoot) {}
